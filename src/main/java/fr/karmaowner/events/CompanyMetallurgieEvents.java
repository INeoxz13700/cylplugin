...
public class CompanyMetallurgieEvents implements Listener {
  ...
  public void CraftingItems(CraftItemEvent e) {
    ...
    if(Player.hasCompany()) {
      ...
      if(company.getCompany() instanceof CompanyMenuiserie)
      {
      ...
      else if (company.getCompany() instanceof CompanyMetallurgie) {
        if (!(company.getCompany()).isThatRegion) {
          ...
          for (ProtectedRegion r : rgs) {
            if (!r.getId().contains("metallurgie")) {
          ...
          if (e.getClickedInventory().getType() != InventoryType.CRAFTING) {
            ...
            if(item == null) item = cm.toXP(id,data);
