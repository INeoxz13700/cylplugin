...
public class FouilleTask extends GeneralType {
  private int count;
  ...
  public FouilleTask(int count, String desc) {
    super(desc);
    this.count = count;
  }
  
  public FouilleTask(int count, String desc, long duration) {
    super(desc, duration);
    this.count = count;
  }
  
  public FouilleTask(int count, String desc, Jobs.Job[] job) {
    this(count, desc);
    this.job = job;
  }
  
  public FouilleTask(int count, String desc, Jobs.Job[] job, long duration) {
  ...
  public void startTask(final Player p) {
    ...
    final FouilleTask kt = (FouilleTask)missions.getInProgress().getType();
    startMission();
    ...
    setTask(new TaskCreator(new CustomRunnable() {
            public void customRun() {
              if (Bukkit.getPlayerExact(p.getName()) == null) {
                FouilleTask.this.failed(p);
                cancel();
                return;
              } 
              if (kt.getTempCount() <= 0)
                kt.setFinished(true); 
              if (!FouilleTask.this.isFinished()) {
                if (FouilleTask.this.getDuration() > 0L) {
                  Timestamp now = new Timestamp(System.currentTimeMillis());
                  if (now.getTime() - FouilleTask.this.getStart().getTime() >= FouilleTask.this.getDuration()) {
                    FouilleTask.this.failed(p);
                    cancel();
                  } 
                } 
              } else {
                FouilleTask.this.accomplished(p);
                cancel();
              } 
            }
          },  false, 0L, 20L));
  ...
  public void informations(String playername) {
    ...
    if (p != null) {
      ...
      p.sendMessage(ChatColor.RED + "à fouiller:");
  ...
  public static FouilleTask loadData(String key) {
    ...
    int count = f.getInt(key + ".FouilleTask.count");
    List<String> jobs = f.getStringList(key + ".FouilleTask.jobs");
    if (duration == 0L)
      return new FouilleTask(count, desc, Jobs.Job.getJobs(jobs)); 
    return new FouilleTask(count, desc, Jobs.Job.getJobs(jobs), duration);
  }
  
  public static void saveData(String key, HashMap<String, Object> infos) {
    FileConfiguration f = Main.INSTANCE.getConfig();
    Object jobs = infos.get("jobs");
    f.set(key + ".FouilleTask.jobs", jobs);
    int count = (Integer) infos.get("count");
    f.set(key + ".FouilleTask.count", count);
  }
  
  public void saveData(String key) {
    super.saveData(key);
    FileConfiguration f = Main.INSTANCE.getConfig();
    f.set(key + ".GeneralType.task", "FouilleTask");
    List<String> listJobs = new ArrayList<>();
    for (Jobs.Job j : this.job)
      listJobs.add(j.getName()); 
    f.set(key + ".FouilleTask.jobs", listJobs);
    f.set(key + ".FouilleTask.count", this.count);
  }
}
